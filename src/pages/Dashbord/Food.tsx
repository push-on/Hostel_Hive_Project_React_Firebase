	...
	return (
		<div>
			<nav>
				...
				<ul>
					<li>
						<button onClick={() => setCreateRoomMode(!createRoomMode)} >Create Food Item</button>
						{createRoomMode && <CreateFoodItem createMode={setCreateRoomMode} updateData={get_food_items} />}
					</li>
				</ul>
			</nav>
			<h2>Food Items</h2>
			...
			<dialog open >
				<article>
					...
					<form >
						...
						<input type="text" required onChange={(e) => setFoodName(e.target.value)} />
						...
						<fieldset>
							...
							<input type="radio" id="radio-1" name="radio" onChange={() => setAvailability(true)} value="radio-1" />
							<label htmlFor="radio-1">Yes</label>
							<input type="radio" id="radio-2" name="radio" onChange={() => setAvailability(false)} value="radio-2" />
