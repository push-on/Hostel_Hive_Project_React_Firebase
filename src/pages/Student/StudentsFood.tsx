...
export default function StudentsFood() {
  ...
  const OrderFood = (e: React.FormEvent) => {
    ...
    const selectedFoodItem = FoodItem.find(
      (item: any) => item.id === selectedFood
    )
    if (!selectedFoodItem || !selectedFoodItem.availability) {
      toast.error(
        "Selected food is not available. Please choose an available food item."
      )
      return
    }
  ...
  return (
    <div>
      {currentOrderData && (
        <motion.article
          ...
          <p>
            ...
            {getFoodNameById(currentOrderData?.food_item_id)}
          </p>
          <p>
            <strong>Payment Status:</strong>{" "}
            {currentOrderData?.status ? "Complited" : "Pending"}
          </p>
          <p>
            <strong>Sub Start Date:</strong>{" "}
            {currentOrderData?.subscription_start_date}
          </p>
          <p>
            <strong>Sub End Date:</strong>{" "}
            {currentOrderData?.subscription_end_date}
          </p>
          <nav>
            <ul></ul>
      ...
      <p>
        ...
        {selectedFood ? getFoodNameById(selectedFood) : "No Food Selected"}
      </p>
      <h2>Food Items</h2>
      <motion.table
        initial={{ x: "100vw", opacity: 0 }}
